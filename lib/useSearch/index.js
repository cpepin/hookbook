...
function useSearch(
  ...
  const cancelableFunction = useMemo(() => CustomPromise.fn(onQueryChange), [onQueryChange]);
  const getResults = useMemoize(cancelableFunction);

  const hideResults = useCallback(() => setAreResultsShown(false), [setAreResultsShown]);
  const clearResults = () => setResults([]);
  const setQueryWithoutRefetch = useCallback(
    newQuery => {
      setIsFetchingEnabled(false);
      setQuery(newQuery);
    },
    [setIsFetchingEnabled, setQuery],
  );
  ...
  const onFocus = useCallback(() => {
    if (resolveOnFocus && debouncedQuery && debouncedQuery.length > 0 && isFetchingEnabled) {
